...
function loginAs(user,profile){currentUser={...user,market:profile.market_name,name:profile.coordinator_name,slug:profile.slug};state.vendors=[];state.markets=[];state.expandedRows={};state.approvedToday=0;state.filterPayment='';document.getElementById('auth-screen').style.display='none';document.getElementById('dashboard').style.display='block';document.getElementById('nav-brand').innerHTML='<span>'+esc(profile.market_name)+'</span> &ndash; Dashboard';document.getElementById('nav-user').textContent=profile.coordinator_name;document.getElementById('pub-title').textContent=profile.market_name;renderPublicLinkBanner();renderPending();updateMetrics();}
